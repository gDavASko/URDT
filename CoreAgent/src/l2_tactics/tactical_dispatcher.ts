...
export class TacticalDispatcher {
  private readonly htnBt: HtnBtExecutive;
  private readonly arbiter: MicroSlmArbiter;
  private readonly stagnationPolicy: StagnationPolicy;
  private readonly touchManager: MultiTouchDispatcher;
  private readonly ticker: FlashHoganTicker;

  private stuckCount = 0;
  private lastStateTimestamp = Date.now();
  private lastStateHash = '';

  constructor(
    modelPath?: string,
    touchManager?: MultiTouchDispatcher,
    ticker?: FlashHoganTicker
  ) {
  ...
  public getHtnExecutive(): HtnBtExecutive {
    return this.htnBt;
  }
   ...
   */
  public async evaluateTacticalStep(
    ctx: TacticalContext,
    currentStateHash: string,
    macroGoal = 'Progress through active scene'
  ): Promise<TacticalEvaluationResult> {
